...
# See https://github.com/clippybot/clippybot/issues/585
...
CONFIG_DIR = Path(os.getenv("clippybot_CONFIG_DIR", PACKAGE_DIR.parent / "config"))
assert CONFIG_DIR.is_dir(), CONFIG_DIR

TOOLS_DIR = Path(os.getenv("clippybot_TOOLS_DIR", PACKAGE_DIR.parent / "tools"))
...
def impose_rex_lower_bound() -> None:
    ...
    if version.parse(rex_version) < version.parse(SWEREX_RECOMMENDED_VERSION):
        ...
        get_logger("clippybot").warning(msg)


impose_rex_lower_bound()
get_logger("clippybot").info(get_agent_version_info())
