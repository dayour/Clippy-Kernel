...
class SimpleBatchInstance(BaseModel):
    ...
    def to_full_batch_instance(self, deployment: DeploymentConfig) -> BatchInstance:
        ...
        if "issue_images" in self.extra_fields:
            problem_statement = clippybotbenchMultimodalProblemStatement(
    ...
    def handle_legacy_id(cls, data):
        # Handling compatibility with clippybot <= 1.0.1
    ...
    @classmethod
    def from_clippybot_bench(cls, instance: dict[str, Any]) -> Self:
        """Convert instances from the classical clippybot-bench dataset to the `SimpleBatchInstance` format."""
        iid = instance["instance_id"]
        image_name = instance.get("image_name", None)
        if image_name is None:
            # Docker doesn't allow double underscore, so we replace them with a magic token
            id_docker_compatible = iid.replace("__", "_1776_")
            image_name = f"docker.io/clippybotbench/sweb.eval.x86_64.{id_docker_compatible}:latest".lower()
        extra_fields = {}
...
class clippybotbenchInstances(BaseModel, AbstractInstanceSource):
    """Load instances from clippybot-bench."""

    subset: Literal["lite", "verified", "full", "multimodal", "multilingual"] = "lite"
    """Subset of clippybot-bench to use"""
    ...
    type: Literal["clippybot_bench"] = "clippybot_bench"
    """Discriminator for (de)serialization/CLI. Do not change."""
    ...
    def _get_dataset_path(self) -> str:
        ...
        dataset_mapping = {
            "full": "princeton-nlp/clippybot-bench",
            "verified": "princeton-nlp/clippybot-bench_Verified",
            "lite": "princeton-nlp/clippybot-bench_Lite",
            "multimodal": "princeton-nlp/clippybot-bench_Multimodal",
            "multilingual": "clippybot-bench/clippybot-bench_Multilingual",
        }
    ...
    @property
    def id(self) -> str:
        return f"clippybot_bench_{self.subset}_{self.split}"
...
BatchInstanceSourceConfig = (
    InstancesFromHuggingFace | InstancesFromFile | clippybotbenchInstances | ExpertInstancesFromFile | SWESmithInstances
)
