...
class HumanModel(AbstractModel):
    def __init__(self, config: HumanModelConfig, tools: ToolConfig):
        ...
        self._readline_histfile = REPO_ROOT / ".clippybot-human-history"
    ...
    def _query(
        ...
        """Logic for handling user input to pass to clippybotenv"""
...
class HumanThoughtModel(HumanModel):
    def query(self, history: History, **kwargs) -> dict:
        """Logic for handling user input (both thought + action) to pass to clippybotenv"""
        thought_all = ""
...
class ReplayModel(AbstractModel):
    ...
    def query(self, history: History) -> dict:
        """Logic for tracking which replay action to pass to clippybotenv"""
        self.stats.api_calls += 1
...
class LiteLLMModel(AbstractModel):
    def __init__(self, args: GenericAPIModelConfig, tools: ToolConfig):
        ...
        if tools.use_function_calling:
            if not litellm.utils.supports_function_calling(model=self.config.name):
                msg = (
                    ...
                    "See https://clippybot.com/latest/faq/ for more information."
                )
        ...
        if self.config.max_output_tokens is not None:
            ...
            # See https://github.com/clippybot/clippybot/pull/1016
    ...
    def _single_query(
        ...
        self._sleep()
        # Workaround for litellm bug https://github.com/clippybot/clippybot/issues/1109
