...
clippybot runs on docker images (`python:3.11` by default).
...
Build it with `docker build -f tiny.Dockerfile -t clippybot-tiny .`.

Now you can run it in the agent with `clippybot run --env.deployment.image clippybot-tiny ...`
