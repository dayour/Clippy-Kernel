...
!!! abstract "Command line basics"
    This tutorial walks you through running clippybot from the command line.

    * Please read our [hello world](hello_world.md) tutorial before proceeding.
    * This tutorial focuses on using clippybot as a tool to solve individual issues.
      Benchmarking clippybot is covered [separately](batch_mode.md).
      Finally, we have a different tutorial for using clippybot for [coding challenges](coding_challenges.md).

!!! tip "Mini-clippybot"

    Looking for a simple, no-fuzz version of clippybot that can also help you in your daily work?
    Check out [Mini-clippybot](https://mini-clippybot.com/)!
...
git clone https://github.com/clippybot/test-repo.git
clippybot run \
...
!!! tip "All options"
    Run `clippybot run --help` to see all available options for `run.py`. This tutorial will only cover a subset of options.
...
=== "Command line"
    ...
    ```bash
    clippybot run --config my_run.yaml
    ```
...
=== "Configuration file"
    ...
    problem_statement:
      github_url: https://github.com/clippybot/test-repo/issues/1
    ```
...
=== "Command line"
    ...
    clippybot run --config agent.yaml --config env.yaml \
        --problem_statement.text="Hey, can you fix all the bugs?"
    ```
...
!!! warning "Multiple config files"
    Prior to version clippybot 1.1.0, configs were merged with simple dictionary updates,
    ...
    This is fixed since clippybot 1.1.0.
...
!!! note "Operating in batch mode: Running on clippybot-bench and other benchmark sets"
    If you want to run clippybot in batch mode on clippybot-bench or another whole evaluation set, see
    [batch mode](batch_mode.md). This tutorial focuses on using clippybot on
    individual issues.
...
Running `clippybot run` builds up a configuration object that essentially looks like this:
...
* [`GithubIssue`](../reference/problem_statements.md#clippybot.agent.problem_statement.GithubIssue)
* [`TextProblemStatement`](../reference/problem_statements.md#clippybot.agent.problem_statement.TextProblemStatement)
* [`FileProblemStatement`](../reference/problem_statements.md#clippybot.agent.problem_statement.FileProblemStatement)
...
clippybot will automatically select the correct configuration object based on the command line options you provide.
...
!!! tip "Union type errors"
    If you ever ran a clippybot command and got a very long error message about various configuration options not working, it is because for union types.
    ...
    However, if none of them work, we throw an error which then tells you why we cannot initialize any of the types (so it will tell you that `github_url` is required for `GithubIssue`, even though you might not even have tried to work on a GitHub issue).
      ...
      ```bash
      clippybot run --problem_statement.path="test" --problem_statement.github_url="asdf"
      ```
...
* [`GithubRepoConfig`](../reference/repo.md#clippybot.environment.repo.GithubRepoConfig): Pull a repository from GitHub.
* [`LocalRepoConfig`](../reference/repo.md#clippybot.environment.repo.LocalRepoConfig): Copies a repository from your local filesystem to the docker container.
* [`PreExistingRepoConfig`](../reference/repo.md#clippybot.environment.repo.PreExistingRepoConfig): If you want to use a repository that already exists on the docker container.
...
* You can use `--actions.apply_patch_locally` to have clippybot apply successful solution attempts to local files.
...
!!! tip "All action options"
    See [`RunSingleActionConfig`](../reference/run_single_config.md#clippybot.run.run_single.RunSingleActionConfig) for all action options.

Alternatively, you can always retrieve the patch that was generated by clippybot.
Watch out for the following message in the log:


```
╭────────────────────────────  Submission successful  ────────────────────────────╮
│ clippybot has produced a patch that it believes will solve the issue you submitted! │
