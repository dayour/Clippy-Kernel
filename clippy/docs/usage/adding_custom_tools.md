...
!!! abstract "Adding custom tools to clippybot"
    This tutorial walks you through creating and integrating custom tools into clippybot.
...
Every clippybot tool is organized as a "bundle" - a directory containing:
...
Create the `config.yaml` file that tells clippybot about your tool:
...
Now you need to tell clippybot to use your new tool. Copy `config/default.yaml` to `config/my_custom_config.yaml`
...
Now you can test your tool by running clippybot with your custom configuration:
