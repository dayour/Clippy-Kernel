...
If you forked or branched off before these changes and now try to synchronize your fork/branch with `clippybot/clippybot:main`, you will
...
```bash
git remote add upstream https://github.com/clippybot/clippybot.git
git fetch upstream
```

Now, you need the updated `pyproject.toml` and `.pre-commit-config.yaml` files.
We can get them from `clippybot/clippybot:main`:
