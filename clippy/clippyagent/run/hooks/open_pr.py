...
def open_pr(*, logger, token, env: clippybotenv, github_url, trajectory, _dry_run: bool = False) -> None:
    ...
    branch_name = f"clippybot-fix-#{issue.number}-" + str(random.random())[2:10]
    env.communicate(
        input="git config user.email 'noemail@clippybot.com' && git config user.name 'clippybot'",
    ...
    body = (
        f"This is a PR opened by AI tool [clippy agent](https://github.com/clippybot/clippybot/) "
    ...
    if not _dry_run:
        args = dict(
            owner=owner,
            repo=repo,
            title=f"clippybot[bot] PR to fix: {issue.title}",
            head=head,
            base=default_branch,
            body=body,
            draft=True,
...
def format_trajectory_markdown(trajectory: list[dict[str, str]], char_limit: int | None = None):
    ...
    prefix = [
        "<details>",
        "<summary>Thought process ('trajectory') of clippybot (click to expand)</summary>",
        "",
        "",
    ]
    prefix_text = "\n".join(prefix)
