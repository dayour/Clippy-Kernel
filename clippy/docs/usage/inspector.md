...
!!! abstract "Inspecting trajectories"
    * Trajectories are the main output of clippybot. They are the best way to understand what clippybot does, especially when running on many problem instances.
    * We provide two tools for visualizing the [`.traj` files](trajectories.md) from the `trajectories` folder more easily.
    * Use `clippybot inspect` (or `clippybot i`) to open the command line inspector.
    * Use `clippybot inspector` (or `clippybot I`) to open the web inspector.
...
1. Don't have a folder here? Make sure to run clippybot at least once.
...
If you are running clippybot on a benchmark (see [batch mode](batch_mode.md)), you will see evaluation results as PASS or FAIL.
Otherwise, you will see a question mark.

!!! tip
    * If you do not see evaluation results, make sure that the clippybot-bench output
      is called `results.json` and is in the same directory as the trajectories.
    * To see gold patches, point `--data_path` to the clippybot-bench dataset.
