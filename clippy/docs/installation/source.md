...
Installation from source is the preferred way to set up clippybot on your machine.

1. Clone the repository, for example with
    ```bash
    git clone https://github.com/clippybot/clippybot.git
    ```
2. Run
    ```
...
You might also try <code>python -m clippybot</code>. If this also doesn't work,
please check with <code>which python</code> that you're using the same python as
when you installed clippybot.

</details>

**Optional installation steps:**

1. The default backend for clippybot is docker, so we recommend to install Docker
   ...
   If you do not want to use docker, you can still use clippybot with code evaluation in the cloud.
...
!!! tip "Installation tips"
    ...
    * clippybot is still in active development. Features and enhancement are added often.
    To make sure you are on the latest version, periodically run `git pull`
    (there is no need to redo the `pip install`).
    * clippybot EnIGMA is currently only compatible with `v0.7` of clippybot. Please run `git switch v0.7` after step 1 to switch to the correct version.
    * Want to modify clippybot? Great! There are a few extra steps and tips:
