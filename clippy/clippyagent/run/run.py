"""[cyan][bold]Main command line interface for clippybot.[/bold][/cyan]

[cyan][bold]=== USAGE ===[/bold][/cyan]

[green]clippybot <command> [options][/green]

Display usage instructions for a specific command:

[green]clippybot <command> [bold]--help[/bold][/green]

[cyan][bold]=== SUBCOMMANDS TO RUN clippybot ===[/bold][/cyan]

[bold][green]run[/green][/bold] or [bold][green]r[/green][/bold]: Run clippybot on a single problem statement, for example a github issue.
[bold][green]run-batch[/green][/bold] or [bold][green]b[/green][/bold]: Run clippybot on a batch of problem statements, e.g., on clippybot-bench.
...
[bold][green]run-api[/green][/bold]: Run clippybot as a backend for a GUI
...
def main(args: list[str] | None = None):
    ...
    if command in ["run", "r"]:
        from clippybot.run.run_single import run_from_cli as run_single_main

        run_single_main(remaining_args)
    elif command in ["run-batch", "b"]:
        from clippybot.run.run_batch import run_from_cli as run_batch_main

        run_batch_main(remaining_args)
    elif command == "run-replay":
        from clippybot.run.run_replay import run_from_cli as run_replay_main

        run_replay_main(remaining_args)
    elif command == "traj-to-demo":
        from clippybot.run.run_traj_to_demo import run_from_cli as convert_traj_to_demo_main

        convert_traj_to_demo_main(remaining_args)
    elif command == "run-api":
        from clippybot.api.server import run_from_cli as run_api_main

        run_api_main(remaining_args)
    elif command == "merge-preds":
        from clippybot.run.merge_predictions import run_from_cli as merge_predictions_main

        merge_predictions_main(remaining_args)
    elif command in ["inspector", "I"]:
        from clippybot.inspector.server import run_from_cli as inspector_main

        inspector_main(remaining_args)
    elif command in ["inspect", "i"]:
        from clippybot.run.inspector_cli import main as inspect_main

        inspect_main(remaining_args)
    elif command == "extract-pred":
        from clippybot.run.extract_pred import run_from_cli as extract_pred_main

        extract_pred_main(remaining_args)
    elif command in ["compare-runs", "cr"]:
        from clippybot.run.compare_runs import run_from_cli as compare_runs_main

        compare_runs_main(remaining_args)
    elif command in ["remove-unfinished", "ru"]:
        from clippybot.run.remove_unfinished import run_from_cli as remove_unfinished_main

        remove_unfinished_main(remaining_args)
    elif command in ["quick-stats", "qs"]:
        from clippybot.run.quick_stats import run_from_cli as quick_stats_main

        quick_stats_main(remaining_args)
    elif command in ["shell", "sh"]:
        from clippybot.run.run_shell import run_from_cli as run_shell_main
