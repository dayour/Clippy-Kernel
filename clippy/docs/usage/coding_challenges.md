# Using clippybot for coding challenges

!!! abstract "Overview"
    It is easy to use clippybot to do more than just software engineering.
    For example, you can tell clippybot to work on leetcode or humaneval-style problems.

    Required reading:
    Please first skim [hello world](hello_world.md) and [command line basics](cl_tutorial.md) to get familiar with the basics of clippybot.
...
Second, we need to specify a repository wherein clippybot will work.
Here, we can simply create an empty folder (outside of the clippybot repository), and add a `main.py` file
...
!!! tip
    ...
    in leetcode) , clippybot will figure out how to add them. However, it might take an
...
Now, we can let clippybot solve the problem:
...
clippybot will typically conclude with a message like
...
╰─────────────────────────────────────────────────────────────────────────────────────╯
 ...
 PATCH_FILE_PATH='/Users/fuchur/Documents/24/git_sync/clippybot/trajectories/fuchur/azure-gpt4__problem__coding_challenge__t-0.00__p-0
...
### Improving clippybot for coding challenges
