...
!!! abstract "Running clippybot competitively on benchmarks"
    This page contains information on our competitive runs on clippybot-bench, as well as tips and tricks for evaluating on large batches.
...
!!! hint "Most recent configs"
    You can find all benchmark submission configs [here](https://github.com/clippybot/clippybot/tree/main/config/benchmarks)

Examples of configurations for clippybot-bench submissions:

* [250225_anthropic_filemap_simple_review.yaml](https://github.com/clippybot/clippybot/blob/main/config/250225_anthropic_filemap_simple_review.yaml):
  This is our current default one-attempt config. It uses `claude-3-7-sonnet-20250219`.
* [250212_clippybot_heavy_sbl.yaml](https://github.com/clippybot/clippybot/blob/main/config/250212_clippybot_heavy_sbl.yaml):
...
defense to kill any process that hogs too much memory (note that this will affect _any_ script and not just clippybot):
...
If clippybot dies or you frequently abort it, you might have leftover docker containers
(they are cleaned up by normal termination of clippybot but can be left over if it is killed).
You can use a sentinel script like the following to clean them up periodically
(note that this will affect _any_ long running container and not just those from clippybot):
...
* The simplest setting to keep cost in check is the per instance cost limit or turn limit.
  ...
  For our clippybot-bench submissions we use slightly higher limits (see the configs above).
