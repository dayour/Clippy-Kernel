...
class WindowsClippyMCPDeployer:
    ...
    def setup_python_environment(self, use_uv: bool = True) -> bool:
        ...
        try:
            if use_uv:
                ...
                subprocess.run(
                    ["uv", "pip", "install", "-e", ".[windows-clippy-mcp]"], cwd=self.project_root, check=True
                )

                print("✓ Python environment set up with UV")
            else:
                print("Setting up Python environment with pip...")
                subprocess.run(["pip", "install", "-e", ".[windows-clippy-mcp]"], cwd=self.project_root, check=True)
    ...
    def setup_vscode_extension(self) -> bool:
        ...
        try:
            ...
            subprocess.run(["npm", "install"], cwd=self.vscode_extension_dir, check=True)

            # Compile TypeScript
            subprocess.run(["npm", "run", "compile"], cwd=self.vscode_extension_dir, check=True)
...
'''
            ...
            default_config = {
                ...
                "shared_llm_keys": {"openai": "OPENAI_API_KEY", "anthropic": "ANTHROPIC_API_KEY"},
            }
    ...
    def setup_azure_integration(
        self, tenant_id: str | None = None, client_id: str | None = None, vault_url: str | None = None
    ) -> bool:
...
"""
    ...
    def deploy(
        self, use_uv: bool = True, setup_vscode: bool = True, setup_azure: bool = True, mcp_port: int = 8765
    ) -> bool:
...
def main():
    ...
    parser.add_argument("--no-uv", action="store_true", help="Use pip instead of UV for Python packages")
    parser.add_argument("--no-vscode", action="store_true", help="Skip VSCode extension setup")
    parser.add_argument("--no-azure", action="store_true", help="Skip Azure integration setup")
    parser.add_argument("--port", type=int, default=8765, help="Port for MCP server (default: 8765)")
    parser.add_argument(
    ...
    success = deployer.deploy(
        use_uv=not args.no_uv, setup_vscode=not args.no_vscode, setup_azure=not args.no_azure, mcp_port=args.port
    )
