...
def _jsonify_paths(value):
    """Convert Path values in nested config data to JSON-serializable strings."""
...
def _load_agent_config(config_path: str | None, base_config: ClippySWEConfig | None = None) -> ClippySWEConfig:
    """Load an agent config file and merge it onto an optional base configuration."""
    if not config_path:
        return base_config or ClippySWEConfig()
    ...
    try:
        ...
        raise ValueError(f"Configuration file not found: {config_file}") from e
    except json.JSONDecodeError as e:
        raise ValueError(f"Configuration file is not valid JSON: {config_file} ({e})") from e

    if not isinstance(custom_config, dict):
        raise ValueError(f"Configuration file must contain a JSON object: {config_file}")

    merged_config = {}
    if base_config is not None:
        merged_config.update(base_config.model_dump())
    merged_config.update(custom_config)

    try:
        return ClippySWEConfig(**merged_config)
    except Exception as e:
        raise ValueError(f"Configuration file is invalid: {config_file} ({e})") from e
...
def task(
    ...
    config: str | None = typer.Option(None, "--config", "-c", help="Path to custom configuration file"),
    ...
    background: bool = typer.Option(
        ...
        "--background",
        "-b",
        help="Reduce foreground output; execution remains synchronous",
    ),
    ...
    try:
        ...
        if config:
            agent_config = _load_agent_config(config, base_config=agent_config)
...
def windows(
    ...
    app_name: str | None = typer.Option(None, "--app", "-a", help="Application name to interact with"),
...
def status(
    ...
    try:
        ...
        table.add_row("Agent Initialized", "yes" if agent_info.get("initialized") else "no")
        table.add_row("LLM Configured", "yes" if agent_info.get("llm_configured") else "no")
        table.add_row("Autonomous Mode", "yes" if agent_info.get("autonomous_mode") else "no")
        table.add_row("Tool Count", str(agent_info.get("tool_count", 0)))
        table.add_row("WorkIQ Enabled", "yes" if agent_info.get("workiq_enabled") else "no")
        table.add_row("M365 Copilot Enabled", "yes" if agent_info.get("m365_copilot_enabled") else "no")
        ...
        if recent_tasks and verbose:
            ...
            for task in recent_tasks:
                status_marker = "done" if task["status"] == "completed" else "failed"
                console.print(f"  {status_marker} [{task['type']}] Task #{task['id']} - {task['status']}")
...
def history(
    ...
    task_id: int | None = typer.Option(None, "--id", help="Show specific task by ID"),
    ...
    try:
        ...
        if task_id:
            ...
            for task in tasks:
                ...
                table.add_row(str(task["id"]), task["type"], task["status"], description, task["timestamp"])
...
def init(
    ...
    try:
        ...
        config_data = _jsonify_paths(default_config.model_dump())
        ...
        console.print(f"  2. Create OAI_CONFIG_LIST at {default_config.llm_config_path}")
        console.print(f"  3. Shared Copilot config root: {default_config.config_dir}")
        console.print("  4. Run: clippy-swe task 'Your task description'")
...
@app.command()
def interactive(
    session_file: Path | None = typer.Option(None, "--session", "-s", help="Session file to save/load"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to custom configuration file"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable verbose logging"),
) -> None:
    """
    ...
    try:
        ...
        session = InteractiveSession(agent, session_file=session_file or agent_config.interactive_session_path)
        session.run()
...
def resolve_issue(
    ...
    issue_number: int = typer.Argument(..., help="Issue number to analyze with the experimental workflow"),
    create_pr: bool = typer.Option(
        True,
        "--create-pr/--no-pr",
        help="Attempt pull request creation if a patch was produced",
    ),
    github_token: str | None = typer.Option(None, "--token", "-t", help="GitHub personal access token"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable verbose logging"),
) -> None:
    """
    Analyze a GitHub issue and run the experimental patch workflow.

    This command currently:
    1. Clones the repository
    2. Analyzes the issue and codebase
    3. Asks the agent for a proposed solution
    4. Heuristically runs common test commands
    ...
    try:
        ...
        # Display results
        if result.success:
            console.print("\n[bold green]Issue workflow completed successfully.[/bold green]")
            ...
            console.print("\n[bold red]Issue workflow failed.[/bold red]")
...
def generate_ppt(
    ...
    subtitle: str | None = typer.Option(None, "--subtitle", "-s", help="Presentation subtitle"),
    generate_images: bool = typer.Option(True, "--generate-images/--no-images", help="Generate images with Flux 2"),
    flux_api_key: str | None = typer.Option(None, "--flux-key", help="Flux 2 API key"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable verbose logging"),
) -> None:
    """
...
def analyze_doc(
    ...
    output: Path | None = typer.Option(None, "--output", "-o", help="Save analysis to file"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable verbose logging"),
) -> None:
    """
...
def create_spec(
    ...
    try:
        ...
        if result["success"]:
            console.print("\n[bold green]Feature specification created.[/bold green]")
            console.print(f"[cyan]Output: {result['output_path']}[/cyan]")
...
def analyze_recording(
    ...
    transcript: Path | None = typer.Option(None, "--transcript", "-t", help="Existing transcript file"),
    output: Path | None = typer.Option(None, "--output", "-o", help="Save analysis to file"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable verbose logging"),
) -> None:
    """
...
def models(
    ...
    set_provider: str | None = typer.Option(None, "--provider", "-p", help="Set default provider"),
    show_current: bool = typer.Option(True, "--current", help="Show current model configuration"),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable verbose logging"),
) -> None:
    """
    Manage local model and provider defaults for the optional client path.
        ...
        if set_model or set_provider:
            ...
            if config_path.exists():
                try:
                    ...
                    console.print(f"[red]Error: Configuration file is invalid JSON: {exc}[/red]")
...
def version() -> None:
    ...
    try:
        ...
        console.print("[dim]Part of clippy kernel, an AG2-based research and development fork[/dim]")
        console.print("[dim]Includes optional Copilot-style client scaffolding[/dim]")
    except Exception:
