...
from clippybot.agent.reviewer import (
    ChooserRetryLoop,
    RetryLoopConfig,
    ReviewSubmission,
    ScoreRetryLoop,
...
from clippybot.tools.parsing import (
    ActionOnlyParser,
    ThoughtActionParser,
)
...
class TemplateConfig(BaseModel):
    ...
    relative to the clippybot_CONFIG_ROOT (if set) or the clippybot repository root
    """
    ...
    """If True, disable image processing for multimodal problem statements (i.e. clippybotbenchMultimodalProblemStatement).
    """
    ...
    def warnings(self) -> Self:
        ...
        if not self.system_template or not self.instance_template:
            logger.warning(
                ...
                " overwriting the default config? See https://clippybot.com/latest/usage/cl_tutorial/"
...
RETRY_WITH_OUTPUT_TOKEN = "###clippybot-RETRY-WITH-OUTPUT###"
RETRY_WITHOUT_OUTPUT_TOKEN = "###clippybot-RETRY-WITHOUT-OUTPUT###"
EXIT_FORFEIT_TOKEN = "###clippybot-EXIT-FORFEIT###"
...
class RetryAgent(AbstractAgent):
    def __init__(self, config: RetryAgentConfig):
        ...
        self._env: clippybotenv | None = None
...
class DefaultAgent(AbstractAgent):
    ...
    def setup(
        ...
        if hasattr(problem_statement, "type") and problem_statement.type == "clippybot_bench_multimodal":
            from clippybot.agent.problem_statement import clippybotbenchMultimodalProblemStatement

            if isinstance(problem_statement, clippybotbenchMultimodalProblemStatement):
        ...
        self.info = AgentInfo()
        self.info["clippybot_hash"] = get_agent_commit_hash()
        self.info["clippybot_version"] = __version__
    ...
    def _add_templated_messages_to_history(
        ...
        # freeze in the agent. See https://github.com/clippybot/clippybot/issues/901 .
