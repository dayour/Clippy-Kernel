...
class ToolHandler:
    ...
    def install(self, env: clippybotenv) -> None:
        self._install_commands(env)
        self.reset(env)

    def reset(self, env: clippybotenv) -> None:
        ...
        env.write_file("/root/.clippybot-env", json.dumps(self.config.registry_variables))
        env.write_file("/root/state.json", "{}")
        env.communicate(" && ".join(self._reset_commands), check="raise", timeout=self.config.install_timeout)

    async def _upload_bundles(self, env: clippybotenv) -> None:
    ...
    async def _check_available_commands(self, env: clippybotenv, env_vars: dict[str, str]) -> None:
        await asyncio.gather(
            *(self._is_command_available(env, command.name, env_vars) for command in self.config.commands)
        )

    def _install_commands(self, env: clippybotenv) -> None:
    ...
    def _get_state(self, env: clippybotenv) -> dict[str, str]:
    ...
    def get_state(self, env: clippybotenv) -> dict[str, str]:
    ...
    def check_for_submission_cmd(self, output: str) -> bool:
        ...
        if r"<<clippybot_SUBMISSION>>" in output:
            return True
        return False
