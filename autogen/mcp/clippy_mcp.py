...
__all__ = ["AzureKeyVaultConfig", "ClippyMCPConfig", "EntraIDConfig", "WindowsClippyMCPClient", "create_clippy_toolkit"]
...
class AzureKeyVaultConfig(BaseModel):
    ...
    certificate_path: str | None = Field(default=None, description="Path to certificate for authentication")
...
class ClippyMCPConfig(BaseModel):
    ...
    working_directory: str | None = Field(default=None, description="Working directory for MCP server")
    environment_variables: dict[str, str] = Field(default_factory=dict, description="Environment variables")

    # Azure integrations
    azure_key_vault: AzureKeyVaultConfig | None = Field(default=None, description="Azure Key Vault configuration")
    entra_id: EntraIDConfig | None = Field(default=None, description="Entra ID configuration")

    # LLM service configuration
    shared_llm_keys: dict[str, str] = Field(default_factory=dict, description="Shared LLM service API keys")
    llm_key_vault_mapping: dict[str, str] = Field(
        default_factory=dict, description="Mapping of LLM services to Key Vault secret names"
    )
...
class WindowsClippyMCPClient:
    ...
    def __init__(self, config: ClippyMCPConfig):
        ...
        self._azure_credentials: Any | None = None
    ...
    async def get_llm_api_key(self, service_name: str) -> str | None:
        ...
        if (
            self.config.azure_key_vault
            and service_name in self.config.llm_key_vault_mapping
    ...
    def create_vscode_extension_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "categories": ["Other"],
            "activationEvents": ["onCommand:ag2-clippy.start", "onLanguage:python"],
            "main": "./out/extension.js",
            "contributes": {
                "commands": [
                    {"command": "ag2-clippy.start", "title": "Start AG2 Clippy Assistant"},
                    {"command": "ag2-clippy.stop", "title": "Stop AG2 Clippy Assistant"},
                    {"command": "ag2-clippy.configure", "title": "Configure AG2 Clippy"},
                ],
                "configuration": {
            ...
            "devDependencies": {"@types/node": "^18.x", "@types/vscode": "^1.80.0", "typescript": "^5.0.0"},
...
async def create_clippy_toolkit(
    ...
    resource_download_folder: Path | None = None,
) -> Toolkit:
...
async def _create_clippy_specific_tools(client: WindowsClippyMCPClient, session: ClientSession) -> list[Tool]:
    ...
    if client.config.memory_engine_enabled:
        ...
        async def save_to_memory(content: str, context: str = "general", agent_id: str | None = None) -> str:
