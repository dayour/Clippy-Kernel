...
class AG2MCPServer:
    ...
    def _register_tools(self):
        ...
        async def create_agent(
            name: str, system_message: str = "You are a helpful AI assistant.", llm_config: dict[str, Any] | None = None
        ) -> CallToolResult:
            ...
            try:
                ...
                agent = ConversableAgent(
                    name=name, system_message=system_message, llm_config=LLMConfig(llm_config), human_input_mode="NEVER"
                )
                ...
                return CallToolResult(
                    content=[TextContent(type="text", text=f"Error creating agent: {str(e)}")], isError=True
                )
        ...
        async def list_agents() -> CallToolResult:
            ...
            return CallToolResult(
                content=[
                    TextContent(
                        type="text", text=f"Available agents: {', '.join(agent_list) if agent_list else 'None'}"
                    )
                ]
            )

        @self.server.call_tool()
        async def send_message(agent_name: str, message: str, recipient_name: str | None = None) -> CallToolResult:
            ...
            try:
                ...
                if recipient_name and recipient_name in self.agents:
                    ...
                    response = await sender.a_send_message(
                        message={"content": message, "role": "user"}, recipient=recipient
                    )
                else:
                    # Direct message to agent
                    response = await sender.a_generate_reply(messages=[{"content": message, "role": "user"}])
                ...
                return CallToolResult(content=[TextContent(type="text", text=str(response))])

            except Exception as e:
                return CallToolResult(
                    content=[TextContent(type="text", text=f"Error sending message: {str(e)}")], isError=True
                )

        @self.server.call_tool()
        async def run_group_chat(
            participant_names: list[str], initial_message: str, max_rounds: int = 5
        ) -> CallToolResult:
            ...
            try:
                ...
                for round_num in range(max_rounds):
                    for i, agent in enumerate(participants):
                        response = await agent.a_generate_reply(messages=[{"content": current_message, "role": "user"}])
                ...
                # Format response
                formatted_log = "\n".join([
                    f"Round {entry['round']} - {entry['agent']}: {entry['response']}" for entry in conversation_log
                ])

                return CallToolResult(
                    content=[TextContent(type="text", text=f"Group chat completed:\n{formatted_log}")]
                )

            except Exception as e:
                return CallToolResult(
                    content=[TextContent(type="text", text=f"Error running group chat: {str(e)}")], isError=True
                )

        @self.server.call_tool()
        async def execute_code(code: str, language: str = "python", agent_name: str | None = None) -> CallToolResult:
            ...
            try:
                ...
                return CallToolResult(
                    content=[TextContent(type="text", text=f"Error executing code: {str(e)}")], isError=True
                )

        @self.server.call_tool()
        async def save_memory(key: str, content: Any, context: str = "general") -> CallToolResult:
            ...
            return CallToolResult(content=[TextContent(type="text", text=f"Saved to memory: {memory_key}")])

        @self.server.call_tool()
        async def retrieve_memory(key: str, context: str = "general") -> CallToolResult:
            """Retrieve content from the memory store."""
            memory_key = f"{context}:{key}"
            if memory_key in self.memory_store:
                content = self.memory_store[memory_key]
                return CallToolResult(
                    content=[TextContent(type="text", text=f"Retrieved from memory: {json.dumps(content, indent=2)}")]
                )
            else:
                return CallToolResult(
    ...
    def _register_resources(self):
        ...
        async def read_resource(uri: str) -> ReadResourceResult:
            ...
            return ReadResourceResult(contents=[TextContent(type="text", text=json.dumps(content, indent=2))])
    ...
    def _register_prompts(self):
        ...
        async def get_prompt(name: str, arguments: dict[str, str] | None = None) -> GetPromptResult:
            ...
            return GetPromptResult(description=f"AG2 prompt template: {name}", messages=[])
    ...
    async def run_stdio(self):
        ...
        try:
            ...
            async with stdio_server() as streams:
                await self.server.run(streams[0], streams[1], init_options={"server_name": self.name})
        except ImportError:
