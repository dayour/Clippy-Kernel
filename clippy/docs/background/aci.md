...
clippybot is built on the idea of an **Agent-Computer Interface** (ACI).
...
The clippybot repository is built to make it easy to invent new ACIs for agents to solve various tasks.

Just like how typical language models requires good prompt engineering, **good ACI design leads to much better results when using agents**. As we show in the clippybot [paper](https://arxiv.org/abs/2405.15793), a baseline agent without a well-tuned ACI does much worse than clippybot.

clippybot contains features that we discovered to be immensely helpful during the agent-computer interface design process:
