...
!!! abstract "Fix a GitHub issue with clippybot"
    In this tutorial, we will fix a GitHub issue with clippybot using the command line interface.

    * Make sure you have [installed](../installation/index.md) clippybot and have a [language model](../installation/keys.md) set up.
    * We will be executing code in a Docker sandbox, so make sure you have docker installed ([docker troubleshooting](../installation/tips.md)).
      If you cannot run docker, skim this tutorial and see how you can run clippybot with cloud-based execution in the [command line basics tutorial](cl_tutorial.md).
...
After installing clippybot, you have the `clippybot` command available. Run `clippybot --help` to see the list of subcommands.
The most important ones are

* `clippybot run`: Run clippybot on a single problem statement. This is covered on this page and for slightly more advanced examples in the [command line basics tutorial](cl_tutorial.md).
* `clippybot run-batch`: Run clippybot on a list of problem statements. This is what you would use for benchmarking, or when
...
Let's start with an absolutely trivial example and solve an issue about a simple syntax error ([`clippybot/test-repo #1`](https://github.com/clippybot/test-repo/issues/1))
...
1. Setting up the **deployment**: clippybot lets LMs execute actions in sandboxed environments. It can run these environments
