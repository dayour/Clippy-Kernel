...
clippybot supports multimodal AI models that can process both text and images. This enables the agent to work with visual context from GitHub issues, such as screenshots, diagrams, and UI mockups.

## Overview

The multimodal implementation automatically:

- **Detects multimodal instances** from clippybot-bench Multimodal datasets
...
Currently, clippybot processes images from the `problem_statement` category, which includes:
...
When loading clippybot-bench instances, multimodal support is automatic:
...
We've enabled multimodal processing when `--instances.type=clippybot-bench --instances.subset=multimodal` are set.
