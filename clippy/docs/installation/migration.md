# clippybot 1.0.0 migration guide

Welcome to clippybot 1.0! So much new stuff! Here's a quick rundown of the cool new things you can do:

* :sparkles: Fast, massively parallel code execution with [SWE-ReX](https://github.com/clippybot/SWE-ReX).
* :sparkles: Run clippybot locally but execute code in the cloud (using modal, AWS, or anything else that runs [SWE-ReX](https://github.com/clippybot/SWE-ReX)).
...
If you're familiar with the old clippybot, here are the main changes you need to be aware of.

!!! tip "clippybot 0.7"
    The old clippybot before the major overhaul is still preserved on the `v0.7` branch.
    In particular, this is still the version to use for the clippybot EnIGMA configuration.
...
* Instead of the `run.py` script, we now have a central `clippybot` entry point that you can call after installing the package.
  This should be much more convenient!
* Instead of one `run.py` command, we now have several subcommands: `clippybot run` to run over single issues, `clippybot run-batch` to run over a whole batch of issues, and various utility commands. Run `clippybot --help` to see all options. Splitting up both commands made it easier to make both use cases more convenient and flexible.
...
* The biggest change is [SWE-ReX](https://github.com/clippybot/SWE-ReX), our new "backend" that handles all code execution.
* As a result of this, the `clippybotenv` class is basically gone and only is a small wrapper around a `swerex` runtime
