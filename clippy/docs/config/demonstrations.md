...
!!! warning "Under construction"
    We're in the process of updating this page to reflect clippybot 1.0.0.
...
For simplicity we only ingest demonstrations in the from of a trajectory file. However, since trajectory files are usually JSON, you can convert them to yaml using the `clippybot traj-to-demo` command to be more human-readable and easier to edit.

Demo (yaml) files are stored in the `demos/` directory by default and consist primarily of the sequence of actions that an LM would need to take to complete a task. It's important that your demo have the proper format to be parsed by clippybot and your config.
...
1. Find a basic trajectory that you already like and want to use as the basis for your demo.
   For instance, consider the `.traj` files in the [`trajectories/demonstrations/` folder](https://github.com/clippybot/clippybot/tree/main/trajectories/demonstrations)
   or find the trajectory from the previous step (the path will be printed at the bottom).
2. Run `clippybot traj-to-demo --traj_path<path to trajectory file.traj>` to convert the trajectory to a demo.
...
4. (Optional) Run `clippybot run-replay --traj_path <path to demo>` to execute the actions of the demo, have the system generate the execution output, and ensure that it works as expected.
