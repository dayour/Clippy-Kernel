"""[cyan][bold]Run clippybot on a single instance taken from github or similar.[/bold][/cyan]
...
clippybot run --config config/default.yaml --agent.model.name "gpt-4o" \\
...
[green]clippybot run --config config/default.yaml --agent.model.name "gpt-4o" \\
...
class RunSingle:
    ...
    def from_config(cls, config: RunSingleConfig) -> Self:
        ...
        self = cls(
            env=clippybotenv.from_config(config.env),
            agent=agent,
