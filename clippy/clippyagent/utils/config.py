...
def _convert_path_relative_to_repo_root(path: Path | str, root: Path | None = None) -> Path | str:
    ...
    root = Path(root or os.getenv("clippybot_CONFIG_ROOT", REPO_ROOT))
...
def _convert_path_to_abspath(path: Path | str) -> Path:
    ...
    using the clippybot_CONFIG_ROOT environment variable (if set) or
    REPO_ROOT as base.
    """
    path = Path(path)
    root = Path(os.getenv("clippybot_CONFIG_ROOT", REPO_ROOT))
    assert root.is_dir()
