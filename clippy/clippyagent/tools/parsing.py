...
!!! note "Describing available tools"
    ...
    Also see [#1130](https://github.com/clippybot/clippybot/issues/1130).
"""
...
from clippybot.exceptions import FormatError, FunctionCallingFormatError
from clippybot.tools.commands import Command
from clippybot.tools.utils import _should_quote
...
class XMLFunctionCallingParser(AbstractParseFunction, BaseModel):
    ...
    def __call__(self, model_response: dict, commands: list[Command], strict=False) -> tuple[str, str]:
        ...
        # Handle different names in clippybot vs. SWE-gym
...
class FunctionCallingParser(AbstractParseFunction, BaseModel):
    ...
    def _parse_tool_call(self, tool_call: dict, commands: list[Command]):
        ...
        def get_quoted_arg(value: Any) -> str:
            ...
            # See https://github.com/clippybot/clippybot/issues/1159
            if value is None:
                return ""
            return value
