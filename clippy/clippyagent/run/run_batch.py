"""
Run on a batch of instances/issues, e.g., clippybot-bench.
...
Basic usage: Run over a [bold][cyan]clippybot-bench lite[/bold][/cyan][green]:

clippybot run-batch \\
    --instances.type clippybot_bench \\ # configure instances
...
[cyan][bold]From huggingface[/bold][/cyan] [green]--instances.type huggingface --instances.dataset_name=clippybot_bench_lite --instances.split=dev[/green].
...
class RunBatchConfig(BaseSettings, cli_implicit_flags=False):
    ...
    def evaluate_and_redo_existing(self) -> Self:
        if not isinstance(self.instances, clippybotbenchInstances):
            return self
...
class RunBatch:
    ...
    def from_config(cls, config: RunBatchConfig) -> Self:
        ...
        if isinstance(config.instances, clippybotbenchInstances) and config.instances.evaluate:
            from clippybot.run.hooks.clippybot_bench_evaluate import clippybotbenchEvaluate

            rb.add_hook(
                clippybotbenchEvaluate(
                    output_dir=config.output_dir,
    ...
    def _run_instance(self, instance: BatchInstance) -> AgentRunResult:
        ...
        env = clippybotenv.from_config(instance.env)
        env.add_hook(
