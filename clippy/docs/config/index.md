...
This page contains details describing how to write your own configurations to control how agents can interact with the `clippybotenv` environment.
...
!!! tip "Default config files"
    Our default config files are in the [`config/`](https://github.com/clippybot/clippybot/tree/main/config) directory.
...
```bash
clippybot run --config config/your_config.yaml
clippybot run-batch --config config/your_config.yaml
```
...
!!! hint "Relative paths"
    Relative paths in config files are resolved to the `clippybot_CONFIG_ROOT` environment variable (if set)
    or the clippybot repository root.

## Multimodal Configuration

For working with images and vision-capable models, clippybot provides specialized multimodal configuration options.
...
- **clippybot-bench Multimodal Image processing**: Downloads and converts GitHub issue images to base64 format for clippybot-bench Multimodal instances.
