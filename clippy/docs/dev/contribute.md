...
Set breakpoints with `breakpoint()` and then run `clippybot` with `pdb`:

```bash
pdb -m clippybot <command> -- <more command line arguments> # (1)!
```

1. Note the `--` before the options passed to clippybot. This is to separate
  options passed to `pdb` from those that are passed to `clippybot`.
...
* When changing the behavior of the agent, we need to have some indication that it actually improves the success rate of clippybot.
  However, if you make the behavior optional without complicating clippybot (for example by providing new [commands](../config/tools.md)),
  we might be less strict.
* Please add simple unit tests or integration tests wherever possible. Take a look in the [tests directory](https://github.com/clippybot/clippybot/tree/main/tests)
  for inspiration. We emphasize simple easy-to-write tests that get a lot of coverage.

[gfi]: https://github.com/clippybot/clippybot/issues?q=is%3Aissue+is%3Aopen+sort%3Aupdated-desc+label%3A%22%F0%9F%91%8B+good+first+issue%22+
[help_wanted]: https://github.com/clippybot/clippybot/issues?q=is%3Aissue+is%3Aopen+sort%3Aupdated-desc+label%3A%22%F0%9F%99%8F+help+wanted%22
