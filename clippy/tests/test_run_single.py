...
def test_run_ies_repo_ps_matrix(
    ...
    if problem_statement_source == "github":
        ps_args = ["--problem_statement.github_url", "https://github.com/clippybot/test-repo/issues/1"]
    elif problem_statement_source == "local":
        ps_args = ["--problem_statement.path", str(clippybot_test_repo_clone / "problem_statements" / "1.md")]
    elif problem_statement_source == "text":
        ps_args = ["--problem_statement.text='this is a test'"]
    else:
        raise ValueError(problem_statement_source)
    if repo == "local":
        repo_args = ["--env.repo.path", str(clippybot_test_repo_clone)]
    elif repo == "github":
        repo_args = ["--env.repo.github_url", "https://github.com/clippybot/test-repo"]
    else:
