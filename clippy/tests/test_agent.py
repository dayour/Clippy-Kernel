...
def default_agent_config():
    config = yaml.safe_load((CONFIG_DIR / "clippybot_0_7/07.yaml").read_text())
...
def test_exit_cost(dummy_env: clippybotenv, test_agent: DefaultAgent, tmp_path):
...
def test_exit_context(dummy_env: clippybotenv, test_agent: DefaultAgent, tmp_path):
...
def test_exit_model_error(dummy_env: clippybotenv, test_agent: DefaultAgent, tmp_path):
...
def test_exit_format(dummy_env: clippybotenv, thought_action_agent: DefaultAgent, tmp_path):
...
def test_exit_blocklist(dummy_env: clippybotenv, test_agent: DefaultAgent, tmp_path):
...
def test_early_exit(dummy_env: clippybotenv, test_agent: DefaultAgent, tmp_path):
...
def test_run_step_by_step_checking_history(dummy_env: clippybotenv, default_agent: DefaultAgent, tmp_path):
    a = default_agent
...
@pytest.mark.xfail
def test_run_autosubmit(dummy_env: clippybotenv, default_agent: DefaultAgent, tmp_path):
    a = default_agent
    ...
    dummy_env.deployment.runtime.run_in_session_outputs = [  # type: ignore
        BashObservation(output=""),
        BashObservation(output=r"<<clippybot_SUBMISSION>>\nmysubmission\n<<clippybot_SUBMISSION>>"),
    ]
    r = a.step()
...
def test_show_no_output_template(dummy_env: clippybotenv, default_agent: DefaultAgent, tmp_path):
    a = default_agent
...
# todo: fixme; Needs real environment or mocking of read_file
@pytest.mark.xfail
def test_successful_submission(dummy_env: clippybotenv, default_agent: DefaultAgent, tmp_path):
    a = default_agent
    a.model = PredeterminedTestModel(["```\nsubmit\n```"])  # type: ignore
    a.setup(dummy_env, EmptyProblemStatement())
    dummy_env.write_file("/root/model.patch", "test")
    dummy_env.deployment.runtime.run_in_session_outputs = BashObservation(output=r"<<clippybot_SUBMISSION>>")  # type: ignore
    a.step()
...
def test_human_exit(dummy_env: clippybotenv, default_agent: DefaultAgent, tmp_path):
    a = default_agent
    a.model = PredeterminedTestModel(["```\nexit\n```"])  # type: ignore
    a.setup(dummy_env, EmptyProblemStatement())
    r = a.step()
...
def test_function_calling(dummy_env: clippybotenv, function_calling_agent: DefaultAgent, tmp_path):
