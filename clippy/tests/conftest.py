...
package_dir = root_dir / "clippybot"
...
def test_trajectory_path(test_trajectories_path) -> Path:
    traj = (
        ...
        / "gpt4__clippybot__test-repo__default_from_url__t-0.00__p-0.95__c-3.00__install-1"
        / "clippybot__test-repo-i1.traj"
    )
    assert traj.exists()
    return traj
...
def test_env_args(
    ...
    clone_cmd = ["git", "clone", "https://github.com/clippybot/test-repo", str(local_repo_path)]
...
@pytest.fixture
def dummy_env(dummy_env_args) -> Generator[clippybotenv, None, None]:
    env = clippybotenv.from_config(dummy_env_args)
    env.start()
    yield env
    env.close()
...
@contextmanager
def clippybot_env_context(env_args):
    """Context manager to make sure we close the shell on the container
    so that we can reuse it.
    """

    env = clippybotenv.from_config(env_args)
    env.start()
    try:
        yield env
    finally:
...
@pytest.fixture
def clippybot_test_repo_clone(tmp_path):
    local_repo_path = tmp_path / "test-repo"
    clone_cmd = ["git", "clone", "https://github.com/clippybot/test-repo", local_repo_path]
    subprocess.run(clone_cmd, check=True)
    return local_repo_path


@pytest.fixture
def clippybot_test_repo_traj(test_trajectories_path) -> Path:
    p = (
        test_trajectories_path
        / "gpt4__clippybot-test-repo__default_from_url__t-0.00__p-0.95__c-3.00__install-1"
        / "6e44b9__clippybottestrepo-1c2844.traj"
    )
    assert p.is_file()
    return p
