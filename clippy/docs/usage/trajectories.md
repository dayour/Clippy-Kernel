...
The [`trajectories/`](https://github.com/clippybot/clippybot/tree/main/trajectories) folder is the default location that experiment results (for example invocations of [`clippybot run`](cl_tutorial.md)) will be written to.

## Trajectories

The main output file is `<instance_id>.traj`, which is a `.json` formatted file containing the (thought, action, observation) turns generated by clippybot towards solving `<instance_id>`.
...
!!! warning "Query and message field"
    Prior to clippybot 1.1.0, we had a `message` field which corresponded (approximately) to the input
...
Note: This trajectory is from clippybot v0.7.0. It might look different in newer versions.
...
Trajectories can also be turned into custom demonstrations for clippybot ([more information](../config/demonstrations.md)).

## Other output files

Together with the trajectory, clippybot will also generate the following files per instance:

* `config.yaml`: A yaml file containing the configuration used for the experiment. You can directly repeat the run of this
  exact instance by running `clippybot run --config config.yaml`.
...
Here are a few more files that are generated with `clippybot run-batch`:
...
* `run_batch.config.yaml`: A yaml file containing the configuration used for the experiment. You can directly repeat the run of this
  exact experiment by running `clippybot run-batch --config run_batch.config.yaml`.
* `preds.json`: All predictions (i.e., patches produced by the agent) from all instances of the experiment.
  If this file goes out of date, you can regenerate it by running `clippybot merge-preds`.
* `run_batch.*.log`: Log files at different levels.
* `run_batch_exit_statuses.yaml`: A yaml file containing the exit statuses of all instances in the experiment (for this run of `clippybot run-batch`).


!!! tip
    * Evaluation is not completed by `clippybot run-batch`, it is a separate step (see [batch mode](batch_mode.md))
    * `preds.json` can be referenced directly into clippybot-bench to run evaluation
    * Trajectories can be turned into custom demonstrations for clippybot ([more information](../config/demonstrations.md)).
